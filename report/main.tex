...
\subsection{Architektura frontendu i komunikacja z API}

Frontend jest podzielony na warstwę routingu, komponenty układu, strony funkcjonalne, kontekst uwierzytelnienia oraz warstwę komunikacji z API. Plik \texttt{App.tsx} definiuje trasy aplikacji, \texttt{AppLayout.tsx} odpowiada za wspólną nawigację i widoczność zakładek zależną od roli, a katalog \texttt{pages} zawiera ekrany obsługujące własny stan formularzy, filtrów, zaznaczonych rekordów, eksportów i odświeżania danych.

Stan globalny ograniczono do uwierzytelnienia. \texttt{AuthProvider} przechowuje token JWT, profil bieżącego użytkownika, stan ładowania oraz komunikaty błędów logowania i rejestracji. Aplikacja nie używa zewnętrznej biblioteki globalnego stanu, ponieważ większość danych jest lokalna dla konkretnych ekranów. Po mutacjach, takich jak utworzenie zgłoszenia, przypisanie recenzenta albo zmiana roli, strona ponownie pobiera dane z API. Dzięki temu przepływ danych pozostaje prosty i czytelny.

Warstwa \texttt{api/client.ts} grupuje wywołania REST i tworzy klienta axios z nagłówkiem \texttt{Authorization: Bearer <token>}. Bazowy adres API pochodzi ze zmiennej \texttt{VITE\_API\_BASE\_URL}, a domyślnie wskazuje \texttt{http://localhost:8080}. Po logowaniu lub rejestracji frontend zapisuje w local storage wyłącznie obiekt \texttt{\{ token \}} pod kluczem \texttt{article-submission-auth}; e-mail i hasło nie są utrwalane. Po starcie aplikacji token jest sprawdzany przez endpoint \texttt{/api/auth/me}. Jeśli żądanie się nie powiedzie, token jest usuwany i użytkownik zostaje lokalnie wylogowany.

Błędy sieciowe i walidacyjne są obsługiwane w miejscu wykonania operacji. Panel logowania odczytuje komunikat \texttt{message} z odpowiedzi backendu, a strony funkcjonalne pokazują komunikaty zastępcze przy nieudanym tworzeniu zgłoszenia, przypisaniu recenzenta, recenzji, zmianie roli albo eksporcie raportów. Nie zastosowano globalnego interceptora błędów axios, ponieważ komunikat zależy od kontekstu konkretnego workflow. Frontend poprawia wygodę użytkownika, ale źródłem prawdy dla reguł biznesowych i autoryzacji pozostaje backend. Ukrywanie zakładek według roli w \texttt{AppLayout.tsx} jest tylko warstwą UX, a nie mechanizmem bezpieczeństwa.
...
Porty hosta są wystawiane wyłącznie na \texttt{127.0.0.1}. Domyślna konfiguracja używa portów \texttt{5432}, \texttt{8080}, \texttt{4173}, \texttt{9090} i \texttt{3000}, ale można je zmienić w pliku \texttt{.env} przez zmienne z sufiksem \texttt{\_HOST\_PORT}. Dzięki temu aplikację można uruchomić także wtedy, gdy jeden z domyślnych portów jest już zajęty na komputerze użytkownika.

Przed pierwszym uruchomieniem należy skopiować plik środowiskowy. W Windows PowerShell używana jest komenda:

\begin{verbatim}
Copy-Item .env.example .env
\end{verbatim}

W macOS i Linux odpowiednikiem jest:

\begin{verbatim}
cp .env.example .env
\end{verbatim}

Następnie cały stos uruchamia się tą samą komendą na Windows, macOS i Linux:

\begin{verbatim}
docker compose up -d --build
\end{verbatim}

Po uruchomieniu z domyślną konfiguracją dostępne są adresy:
...
Projekt posiada testy integracyjne backendu oraz weryfikację frontendu przez lint, build i generowanie zrzutów ekranu. Testy backendu sprawdzają nie tylko start aplikacji, ale także kluczowe reguły biznesowe: edycję szkiców, wyszukiwanie, właścicielstwo zgłoszeń, powtarzalne tytuły u różnych autorów, zarządzanie rolami, workflow recenzji, powiadomienia oraz eksporty raportów.
...
\hline
\texttt{SubmissionServiceIntegrationTest} & Tworzenie, edycja szkicu, wyszukiwanie, filtrowanie, separacja właścicieli i odrzucenie edycji szkicu przez innego autora. \\
\hline
\texttt{WorkflowAndAdministrationIntegrationTest} & Role, przypisania recenzentów, blokada duplikatu przypisania, recenzje, decyzje, powiadomienia i raporty. \\
\hline
\texttt{AuthSecurityIntegrationTest} & Logowanie JWT, profil bieżącego użytkownika, ochrona endpointów tokenem Bearer, odrzucenie niepoprawnego JWT, dostęp do panelu administracyjnego i autoryzacja zmiany ról. \\
\hline
\texttt{npm run lint} & Poprawność statyczna kodu frontendu. \\
\hline
\texttt{npm run build} & Produkcyjna kompilacja React i TypeScript. \\
\hline
\texttt{GitHub Actions CI} & Automatyczne uruchomienie testów backendu oraz lint/build frontendu przy pushach i pull requestach do gałęzi \texttt{main}. \\
\hline
...
Wynik testów backendu: 15 testów, 0 błędów i 0 niepowodzeń. Zestaw obejmuje test uruchomienia kontekstu aplikacji, testy workflow zgłoszeń i administracji, testy granic biznesowych dla właścicieli szkiców, powtarzalnych tytułów i duplikatów przypisań recenzentów, a także testy bezpieczeństwa dla logowania JWT, endpointu bieżącego użytkownika, dostępu bez tokena Bearer, niepoprawnego tokena JWT, dostępu do panelu administratora oraz zmiany ról. Build frontendu oraz lint zakończyły się poprawnie. Dokumentacja obrazkowa została wygenerowana z działającego środowiska Docker, co zwiększa wiarygodność zrzutów ekranu.
